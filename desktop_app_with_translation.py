...
class TextSummarizerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Text Summarizer Pro - Desktop Edition with Translation")
    ...
    def setup_ui(self):
        ...
        subtitle_label = tk.Label(
            title_frame,
            text="Transform your text into concise summaries with translations",
            font=('Arial', 10),
            fg='#7f8c8d',
            bg='#f0f2f6'
        )
        ...
        trans_combo = ttk.Combobox(
            ...
            values=["None", "Hindi", "Marathi", "Spanish", "French", "German", "Japanese", "Chinese", "Arabic"],
            state="readonly",
            width=15
        )
        ...
        self.summary_output.pack(fill='x', padx=15, pady=(0, 10))
    ...
    def update_translation(self, translation):
        ...
        self.trans_output.insert("1.0", f"Translation ({self.trans_var.get()}):\n{translation}")
    ...
    def save_translation(self):
        ...
        if not self.trans_output.get("1.0", tk.END).strip():
        ...
        if file_path:
            try:
                with open(file_path, 'w', encoding='utf-8') as file:
                    file.write(self.trans_output.get("1.0", tk.END))
                    self.status_var.set(f"Translation saved: {file_path}")
            except Exception as e:
                messagebox.showerror("Error", f"Could not save file: {str(e)}")
